...
def vectorize(password):
    ...
    seen = []
    ct_repeat = 0
    max_length_c_letter = 0
    current_length_c_letter = 1
    max_length_c_digit = 0
    current_length_c_digit = 1
    current_length_h_letter = 0
    current_length_t_letter = 0
    current_length_h_digit = 0
    current_length_t_digit = 0
    current_length_h_char = 0
    current_length_t_char = 0
    ...
    for i in range(len(password)):
        ...
        if password[i] >= '0' and password[i] <= '9':
            ct_num += 1
        if ((not (password[i] >= 'a' and password[i] <= 'z')) and (not (password[i] >= 'A' and password[i] <= 'Z')) and (not (password[i] >= '0' and password[i] <= '9'))):
            ct_char += 1
        if password[i] not in seen:
            seen.append(password[i])
        else:
            ct_repeat += 1
        if ord(password[i]) >= 97 and ord(password[i]) <= 122 and ord(password[i-1]) >= 97 and ord(password[i-1]) <= 122:
            current_length_c_letter += 1
        else:
            max_length = max(max_length_c_letter, current_length_c_letter)
            current_length_c_letter = 1
        if ord(password[i]) >= 48 and ord(password[i]) <= 57 and ord(password[i-1]) >= 48 and ord(password[i-1]) <= 57:
            current_length_c_digit += 1
        else:
            max_length_c_digit = max(max_length_c_digit, current_length_c_digit)
            current_length_c_digit = 1
        if ord(password[i]) >= 48 and ord(password[i]) <= 57:
            current_length_h_digit += 1
        if ord(password[len(password)-1-i]) >= 48 and ord(password[len(password)-1-i]) <= 57:
            current_length_t_digit += 1
        if ord(password[i]) >= 97 and ord(password[i]) <= 122:
            current_length_h_letter += 1
        if ord(password[len(password)-1-i]) >= 97 and ord(password[len(password)-1-i]) <= 122:
            current_length_t_letter += 1
        if (not (ord(password[i]) >= 48 and ord(password[i]) <= 57)) and (not (ord(password[i]) >= 65 and ord(password[i]) <= 90)) and (not (ord(password[i]) >= 97 and ord(password[i]) <= 122)):
            current_length_h_char += 1
        if (not (ord(password[len(password)-1-i]) >= 48 and ord(password[len(password)-1-i]) <= 57)) and (not (ord(password[len(password)-1-i]) >= 65 and ord(password[len(password)-1-i]) <= 90)) and (not (ord(password[len(password)-1-i]) >= 97 and ord(password[len(password)-1-i]) <= 122)):
            current_length_t_char += 1
            
    vector.append(len(password))
    vector.append(ct_lower)
    vector.append(ct_upper)
    vector.append(ct_num)
    vector.append(ct_char)
    if ((ct_lower+ct_upper) == len(password)):
        vector.append(1)
    else:
        vector.append(0)
    if (ct_num == len(password)):
        vector.append(1)
    else:
        vector.append(0)
    vector.append(ct_repeat)
    max_length_c_letter = max(max_length_c_letter, current_length_c_letter)
    vector.append(max_length_c_letter)
    max_length_c_digit = max(max_length_c_digit, current_length_c_digit)
    vector.append(max_length_c_digit)
    ...
    if ((ct_lower+ct_upper) == len(password)):
        ...
        vector.append(0)
    vector.append(current_length_h_digit)
    vector.append(current_length_t_digit)
    vector.append(current_length_h_letter)
    vector.append(current_length_t_letter)
    vector.append(current_length_h_char)
    vector.append(current_length_t_char)
...
def sequential_guessor(password):
    ...
    for key in list(guess_class_index.keys()):
        guess_class[guess_class_index[key]] = key
    
    for class_prob in class_order:
        guess_function = guess_functions[guess_class[class_prob]]
        for trans in transformation_mat[guess_class[class_prob]]:
            if guess_class[class_prob] != 'seqkey':
            # if guess_class[class_prob] == 'substring':
                guesses.append(guess_function(password, trans))
            else:
                guesses.append(guess_function(password, trans[0], seq_graph))
    ...
    while alive > 0:
        ...
        # if the class has no more transformation to try and have reported, skip
            ...
            if cur_func == apply_seqkey_transformation:
                cur_guesses = cur_func(password, transformation_mat[guess_classes[class_order[class_idx]]][trans_idx][0],seq_graph)
            else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(success / len(final_test_pairs))
